...
public class Solution {
    public static void main(String... args) throws IOException {
        try (RandomAccessFile raf = new RandomAccessFile(args[0], "rw")) {
            StringBuilder textInArgs = new StringBuilder();
            Arrays.stream(args)
                    .skip(2)
                    .forEach(textInArgs::append);

            raf.seek(Integer.parseInt(args[1]));

            byte[] bytesFromFile = new byte[textInArgs.length()];
            raf.read(bytesFromFile, 0, textInArgs.length());

            StringBuilder textFromFile = new StringBuilder();
            for (byte b : bytesFromFile) {
                textFromFile.append((char) b);
            }

            String isSameText = textInArgs.toString().contentEquals(textFromFile) ? "true" : "false";

            System.out.println(isSameText);
            raf.seek(raf.length());
            raf.write(isSameText.getBytes(StandardCharsets.UTF_8));
        }
    }
}
